<?php 
    ...
    // Crear la consulta para ver la propiedad a modificar
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if(empty($errores)) {
            ...
            // (ANTES CHECAMOS SI SE SUBIO UNA IMAGEN NUEVA)
            if($imagen["name"]) {
                unlink($carpeta_imagenes . $propiedad["imagen"]);
                // Generar un nombre único
                $nombre_imagen = md5(uniqid(rand())) . ".jpg";
                // Subir la imagen a local
                move_uploaded_file($imagen["tmp_name"], $carpeta_imagenes . $nombre_imagen);
            } else {
                $nombre_imagen = $propiedad["imagen"];
            }

            /* Crear el comando SQL para hacer la actualización */
            $query = " UPDATE propiedades SET titulo = '${titulo}', precio = '${precio}', imagen = '${nombre_imagen}',descripcion = '${descripcion}', habitaciones = ${habitaciones}, wc = ${wc}, estacionamiento = ${estacionamiento}, vendedores_id = ${vendedor} WHERE id = ${id}";

            // Insertar en la base de datos
            $resultado = mysqli_query($db, $query);
            if ($resultado) {
                // Redireccionar al usuario   
                header("Location: /admin/index.php?creado=2");
            }
        }
    }
...
?>
        ...
        <main class="contenedor seccion">
            ...
            <form class="formulario" method="POST" enctype="multipart/form-data">
                ...
                <input type="submit" value="Actualizar propiedad" class="boton-verde seccion">
            </form>
        </main>
